...
经 Zotero Local API 把论文导入、把技术文档归档到本地库。一篇论文只需两样:PDF(来自 arXiv)
和元数据(来自权威网源,绝不从 PDF 解析)。分三个阶段:

- **存在性核验(只读)** —— 规范化标识符(DOI / title+authors;arXiv id 只是下载源标识、
  非身份),经 `scholar-workflow zotero` 搜索和回读确认,结果为 exact / conflict / none。
  `zotero ingest` 会在创建前再次执行精确核验。
- **获取元数据(只读联网)** —— 对新条目,从 arXiv abs / CVF / DBLP / 出版社读取元数据;
  有正式发表版本时,会议名覆盖 arXiv 预印本名头;查不到的次要字段留空,不编造。
- **写入(经 Local API)** —— 先问论文归入哪个分类,把 arXiv PDF 下载到 `paper_inbox`,
  再把元数据、collection keys 和 PDF 路径交给 `scholar-workflow zotero ingest`。新增性写入在
  用户已下达指令时直接执行,仅删除/覆盖/合并须批准。技术文档复制进 Vault 并写来源/散列元数据。

下载的论文 PDF 只进 `paper_inbox`,技术文档只进 Vault。所有 Zotero 访问都经 Local API,
